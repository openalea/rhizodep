...
class RootCarbonModel:
    ...
    def total_root_sucrose_and_living_struct_mass(self):
        ...
        for vid in self.g.vertices_iter(scale=1):
            if self.length <= 0.:
                ...
                self.total_sucrose_root += self.C_sucrose_root[vid] * (self.struct_mass[vid] + self.living_root_hairs_struct_mass[vid]) \
    ...
    def shoot_sucrose_supply_and_spreading(self):
        ...
        C_sucrose_root_after_supply = (self.total_sucrose_root[1] + (self.sucrose_input_rate * self.time_steps_in_seconds) - self.global_sucrose_deficit[1]) \
...
class RootGrowthModel:
    ...
    # ------------------------------------------------------------------
    def potential_segment_development(self, g, segment, time_step_in_seconds=60. * 60. * 24., radial_growth="Possible",
    ...
    # ---------------------------
    def nodule_formation(self, g, mother_element):
        """
...
class MTGOutputs:
    ...
    # ----------------------------------------------------------------------------------
    def sub_length_z(self, x1, y1, z1, x2, y2, z2, z_up, z_down):
        """
    ...
    # -----------------------------------------------------------
    def classifying_on_z(self, g, z_min=0., z_max=1., z_interval=0.1):
        """
        ...
        for z_start in np.arange(z_min, z_max, z_interval):
            ...
            name_struct_mass_z = "struct_mass_" + str(round(z_start, 3)) + "-" + str(
            ...
            for vid in g.vertices_iter(scale=1):
                ...
                total_included_net_hexose_exudation += (
                                                                   n.hexose_exudation - n.hexose_uptake_from_soil) * fraction_length
    ...
    # -------------------------------------------------------------
    def recording_MTG_properties(self, g, file_name='g_properties.csv'):
        """
    ...
    def calculating_extra_variables(self, n, time_step_in_seconds):
        """
    ...
    # ---------------------------------------------------------------
    def summing(self, g, printing_total_length=True, printing_total_struct_mass=True, printing_all=False):
        """
        ...
        if printing_all:
            ...
            print(
                "      The current amount of mobile hexose in the roots (including possible deficit and dead roots) is",
            ...
            print(
                "      The current amount of hexose stored as reserve in the roots (including possible deficit and dead roots) is",
