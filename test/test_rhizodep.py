...
def sucrose_to_hexose(g, time=1, Umax_apex=1.8e-5, gamma_unloading=0.4, Km_unloading=6e-6):
    ...
    (in seconds) and its immediate conversion into hexose. 2 mol of hexose are produced for 1 mol of sucrose.
    The unloading of sucrose is represented as an active process with a substrate-limited relationship
    (Michaelis-Menten function), where Umax (in mol s-1) is the maximal amount of sucrose unloading
...
# ---------------------------------------------------------
def soil_hexose_degradation(g, time=1, Dmax=1.8e-5, Km_degradation=6e-6):
    ...
    describing the root system, Dmax is the maximal degradation of hexose (mol m-2), and Km_degradation
    ...
    for vid in g.vertices_iter(scale=1):
        ...
        n.hexose_degradation = n.external_surface * Dmax * n.C_hexose_soil / (Km_degradation + n.C_hexose_soil) * time
