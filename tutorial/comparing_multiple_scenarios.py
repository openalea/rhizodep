...
"""
    This script allows to compare the outputs from different tutorial obtained with running_scenarios.py.
...
def creating_new_frames_from_scenarios(properties=['Root structural mass (g)'],
    ...
    """
    This function is used to compare the outputs of different tutorial at the scale of the whole root system over time.
    The function uses the files 'simulation_results.csv' stored in the folders of each scenario within the main folder
    'outputs'. It generates two csv files for each property, one displaying all values from the different tutorial, and
    the other one showing some statistical results for comparing the outputs of the different tutorial among groups.
    :param properties: a list containing the different properties to be compared among tutorial
    ...
    # We cover each group of tutorial:
    ...
    for property in properties:
        ...
        # We record the final table comparing the values of this specific property among tutorial:
        final_table_name = 'Comparing ' + property + ' for all tutorial.csv'
        ...
        # We now cover each group of tutorial:
        for group in list(scenario_numbers_by_group.keys()):
            # We create a list containing the names of tutorial to be kept for performing the statistics:
...
def plotting_multiple_scenarios(table="Comparing Root structural mass (g).csv", scenario_numbers=[1,2,3,4,5],
    ...
    for different tutorial. It uses a csv file generated by the function 'creating_new_frames_from_scenarios'.
    ...
    # 1) We load one csv file corresponding to the values of one property over time among different tutorial:
...
def plotting_mean_values(table="Comparing Root structural mass (g) for all groups.csv",
    ...
    """
    This function creates a plot showing the evolution of a given property of the whole root system over time,
    averaged among different tutorial. It uses a csv file generated by function 'creating_new_frames_from_scenarios'.
    :param table: the name of the csv file to be read in the folder 'outputs'
    ...
    # 1) We load one csv file corresponding to the mean value of one property for different tutorial over time:
    table_path = os.path.join('outputs', table)
    try:
        df = pd.read_csv(table_path)
    except:
    ...
    # We cover each group of tutorial to be treated together:
    for group in groups:
        mean_name = 'Mean_' + group
        std_name = 'Standard_deviation_' + group
...
if __name__ == '__main__':
    ...
    print(" Extracting the values for different tutorial...")
    ...
    plotting_multiple_scenarios(table="Comparing Root structural mass (g) for all tutorial.csv", scenario_numbers=[1,2,3],
